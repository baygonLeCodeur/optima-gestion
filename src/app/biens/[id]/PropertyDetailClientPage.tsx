...
import { createClient } from '@/lib/supabase/client'; // Importer le client Supabase
...
export default function PropertyDetailClientPage({ property }: { property: Property }) {
  ...
  useEffect(() => {
    const incrementView = async () => {
      ...
      // Maintenant, typedSupabase a le bon type et l'appel RPC est sécurisé.
      const { error } = await typedSupabase.rpc('increment_property_views', {
  ...
  }, [property.id]);// Se déclenche une seule fois
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative">
                <div className="lg:col-span-2 bg-white dark:bg-gray-900 shadow-xl rounded-lg p-6 md:p-8">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-6">
                     <Badge variant="secondary" className="flex items-center gap-2 py-2 px-3"><DoorOpen className="h-4 w-4" /> {property.
      number_of_rooms} Pièces</Badge>
                     <Badge variant="secondary" className="flex items-center gap-2 py-2 px-3"><Bath className="h-4 w-4" /> {property.
      number_of_bathrooms} Salles de bain</Badge>
                     <Badge variant="secondary" className="flex items-center gap-2 py-2 px-3"><SquareM className="h-4 w-4" /> {property.
      area_sqm} m²</Badge>
                  ...
                  {virtualTourData && virtualTourData.scenes && (
                    <section className="mt-8"><h2 className="text-2xl font-bold mb-4">Visite Virtuelle</h2><div className="overflow-hidden 
      rounded-lg"><VirtualTour scenes={virtualTourData.scenes as object} /></div></section>
                  )}
                  <section className="mt-8"><h2 className="text-2xl font-bold mb-4">Description</h2><p className="text-gray-700 
      dark:text-gray-300 whitespace-pre-wrap">{property.description}</p></section>
                  ...
                  {property.latitude && property.longitude && (
                    <section className="mt-8"><h2 className="text-2xl font-bold mb-4">Localisation</h2><div className="overflow-hidden 
      rounded-lg"><Map lat={property.latitude} lng={property.longitude} popupText={property.address} /></div></section>
                  )}
                </div>
                <aside className="relative">
                  <div className="sticky top-24">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl">
                      <p className="text-3xl font-bold text-green-600 mb-6 text-center">{new Intl.NumberFormat('fr-FR', { style: 'currency',
      currency: 'XOF' }).format(property.price)}</p>
    
                      {effectiveAgent ? (
                        user ? (
                          // Si l'utilisateur est connecté, afficher le formulaire de prise de RDV
                          <div>
                            {/* <h3 className="text-xl font-bold mb-4 text-center">Planifier une visite</h3> */}
                            <VisitRequestForm propertyId={property.id} agentId={effectiveAgent.id} />
                          </div>
                        ) : (
                          // Sinon, afficher le formulaire de contact pour les leads
                          <div>
                            <h3 className="text-xl font-bold mb-4 text-center">Contacter l'agent</h3>
                            <AgentContactForm agent={effectiveAgent} propertyId={property.id} />
                          </div>
                        )
                      ) : (
                        <div className="text-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                          <h3 className="text-lg font-semibold mb-2">Informations de contact</h3>
                          <p className="text-gray-600 dark:text-gray-400">Les informations de l'agent ne sont pas disponibles pour le 
      moment.</p>
                          <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">Veuillez nous contacter directement pour plus d
      'informations sur ce bien.</p>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-950/50 py-16 mt-16">
              <div className="container mx-auto px-4"><h2 className="text-3xl font-bold text-center mb-8">Biens Similaires</h2><
      SimilarProperties propertyId={property.id} city={property.city} propertyTypeId={propertyTypeId} price={property.price} area=
